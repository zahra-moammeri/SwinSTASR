...
from swinstasr.archs.transform_blocks import SFB, SWB, STAB
...
class RSTB(nn.Module):
    ...
    def __init__(self,
        ...
        match self.resi_connection:
            case "1conv":
                self.after_body = nn.Conv2d(dim, dim, 3, 1, 1)
            case "SFB":
...
class SwinSTASR(nn.Module):
    r""" SwinWSR
        A PyTorch impl of : `SwinWSR:  Swin Wavelet for Image Super-Resolution`, based on Swin Transformer and Wavelet Convolution.
    ...
    Args:
        ...
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default: False
        upscale: Upscale factor. 2 for image SR
        img_range: Image range. 1. or 255.
        upsampler: The reconstruction reconstruction module. 'pixelshuffle'
    ...
    def __init__(self,
        ...
        match self.resi_connection:
            ...
            case "SWB":
                self.conv_after_body = SWB(embed_dim)
            case "STASR":
                self.conv_after_body = STAB(embed_dim)
            case "identity":
