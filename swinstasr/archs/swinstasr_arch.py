...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        return self.patch_embed(self.after_body(self.patch_unembed(self.residual_group(x, x_size), x_size))) + x


@ARCH_REGISTRY.register()
class SwinSTASR(nn.Module):
    r""" SwinSTASR
        A PyTorch impl of : `SwinSTASR:  Swin Wavelet for Image Super-Resolution`, based on Swin Transformer and Wavelet Convolution.
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.after_body(self.forward_features(x)) + x
